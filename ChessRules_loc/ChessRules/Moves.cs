...
namespace ChessRules
{
    class Moves
    {
        ...
        private bool CanPawnMove()
        {
            ...
            return (CanPawnGo(stepY)   || // ход на +1 клетку
                    CanPawnJump(stepY) || // ход на +2 клетки
                    CanPawnEat(stepY)  ||
                    CanPawnEnPassant(stepY)) && // съесть на проходе
                    CanPawnPromote();
        }

        private bool CanPawnPromote()
        {
            if (fm.to.y == 7) // если мы пошли на 7-ю горизонталь
